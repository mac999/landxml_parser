...
def export_align_block_model_to_json(model, align, json_path):
	...
	"""
	grid_method = 'sta_interval' # 'fixed'  # or 'sta_interval'
	begin_sta = 0.
	end_sta = 0.
	left_side = -3.0
	right_side = 3.0
	blocks_cells, blocks = align.get_alignment_blocks_index().update_blocks_cells(0.5, begin_sta, end_sta, left_side, 0.5, right_side, convert_coord='', grid_method=grid_method)

	with open(json_path, 'w', encoding='utf-8') as f:
		json.dump(blocks_cells, f, ensure_ascii=False, indent=2)

	return blocks_cells, blocks

def query_blocks_in_bbox(align, query_in_bbox):
	# query blocks
	searched_segs = blocks_cells.query_blocks(query_in_bbox)
...
def test_query_performance(aligns):
	...
	align_blocks = aligns[0].get_alignment_blocks_index()
...
def export_blocks():
	parser = lxml.landxml()
	# model = parser.load('./landxml_road_sample.xml') # landxml
	model = parser.load_json('output_dxf_landxml.json') # dxf to landxml json
	print(model)
	...
	export_align_block_model_to_json(cm, aligns[0], 'block_model_output_dxf_sta.json')
	end = time.time()
